...
## Descrição
Desenvolver **tanto uma interface de usuário (UI) quanto um arquivo de configuração persistente (`settings/aclarai.config.yaml`)** para gerenciar as funções de modelo, limiares de similaridade e janelas de contexto do aclarai. A UI deverá sempre refletir o estado atual do arquivo YAML e qualquer alteração na UI deverá ser persistida imediatamente no arquivo, conforme `docs/arch/design_config_panel.md`. Este sistema expandirá o escopo de configuração para incluir todos os aspectos configuráveis do aclarai.

## Escopo

### Incluído
- Desenvolvimento de **uma UI (em Gradio)** e de um **arquivo de configuração (`settings/aclarai.config.yaml`)** para gerenciar as funções de modelo, limiares e janelas, conforme a estrutura e seções propostas em `docs/arch/design_config_panel.md`.
- Implementação da lógica para que a UI **leia e reflita o estado atual** do `settings/aclarai.config.yaml`.
- Implementação da lógica para que as alterações feitas na UI sejam **gravadas de volta no `settings/aclarai.config.yaml`**, garantindo a persistência das configurações.
...
### Excluído
- Implementação de configuração dinâmica em tempo real que permita hot-reloading de modelos ou agentes sem reiniciar os serviços do aclarai. A aplicação de novas configurações persistidas poderá exigir um reinício controlado.
...
- Migração automática de configurações legadas de formatos anteriores (foco no `settings/aclarai.config.yaml` canônico).

## Critérios de Aceitação
- **A UI e o arquivo de configuração (`settings/aclarai.config.yaml`) estão implementados e funcionais.**
- A UI **carrega e reflete** corretamente o estado atual do `settings/aclarai.config.yaml`.
- As alterações feitas na UI são **salvas corretamente** no `settings/aclarai.config.yaml`.
...
- Serviço `aclarai-ui` (Gradio) scaffolded (de Sprint 1).
- Módulos dos serviços aclarai capazes de ler as configurações do arquivo `settings/aclarai.config.yaml`.

## Entregáveis
- Código-fonte da UI (Gradio) para o painel de configuração.
- Módulo para gerenciamento de leitura/escrita do `settings/aclarai.config.yaml`.
- Arquivo `settings/aclarai.config.yaml` de exemplo com configurações padrão.
...
- **Risco**: Inconsistências entre o estado da UI e o conteúdo do `settings/aclarai.config.yaml` devido a edições externas ou falhas de persistência.
...
- **Risco**: Perda de configurações devido a erros de escrita ou sobrescrita acidental.
  - **Mitigação**: Implementar um sistema simples de backup automático do `settings/aclarai.config.yaml` antes de qualquer alteração significativa, e considerar o uso de escrita atômica para o arquivo de configuração, se o sistema operacional permitir.
...
- A persistência das configurações deve ser feita em um arquivo **YAML** (`settings/aclarai.config.yaml`).
