...
from clarifai_shared.embedding import EmbeddingPipeline, EmbeddingResult, VectorStoreMetrics
...
def test_embedding_pipeline_initialization(mock_config):
    ...
    with patch('clarifai_shared.embedding.load_config', return_value=mock_config), \
...
def test_process_single_block_success(mock_config):
    ...
    mock_embedded_chunks = [EmbeddedChunk(mock_chunks[0], [0.1] * 384, "test-model", 384)]
...
def test_process_single_block_with_replacement(mock_config):
    ...
    with patch('clarifai_shared.embedding.load_config', return_value=mock_config), \
        ...
        mock_store = mock_store_class.return_value
        mock_store.delete_chunks_by_block_id.return_value = 2  # 2 existing chunks deleted
        mock_store.store_embeddings.return_value = mock_metrics
        
        pipeline = EmbeddingPipeline(mock_config)
        result = pipeline.process_single_block("New text", "blk_replace", replace_existing=True)
...
def test_search_similar_chunks(mock_config):
    ...
    mock_search_results = [
        ({"clarifai_block_id": "blk_001", "chunk_index": 0, "text": "Similar text 1"}, 0.95),
        ({"clarifai_block_id": "blk_002", "chunk_index": 0, "text": "Similar text 2"}, 0.87)
    ]
