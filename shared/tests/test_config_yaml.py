...
"""
    ...
    with patch('builtins.open', mock_open(read_data=yaml_content)), \
        ...
        mock_yaml_load.return_value = {
            'databases': {
                'postgres': {'host': 'test-postgres', 'port': 5433, 'database': 'test_db'},
            ...
            'concepts': {
                'candidates': {'collection_name': 'test_candidates', 'similarity_threshold': 0.85}
            },
            'paths': {
                'vault': '/test/vault',
                'tier1': 'test_conversations'
...
"""
    ...
    with patch('builtins.open', mock_open(read_data=yaml_content)), \
        ...
        mock_yaml_load.return_value = {
            'databases': {
                'postgres': {'host': 'yaml-postgres', 'port': 5432}
            }
        }
...
def test_yaml_loading_error_handling():
    ...
    with patch('builtins.open', mock_open(read_data="invalid: yaml: content:")), \
         patch('pathlib.Path.exists', return_value=True), \
         patch('clarifai_shared.config.yaml.safe_load', side_effect=Exception("YAML error")):
