...
module_path = os.path.join(os.path.dirname(__file__), '../../clarifai_shared/utils/block_id.py')
...
class TestBlockIdGeneration:
    ...
    def test_generate_unique_block_id_max_attempts(self):
        ...
        with patch.object(random, 'choices', return_value=['a', 'b', 'c', 'd', 'e', 'f']):
            used_ids = {"blk_abcdef"}
            with pytest.raises(RuntimeError, match="Unable to generate unique block ID"):
