...
class TestClaimifyPipeline(unittest.TestCase):
    ...
    def test_context_window_building(self):
        ...
        context = self.pipeline._build_context_window(sentences[-1], sentences, len(sentences) - 1)
    ...
    def test_model_injection(self):
        ...
        mock_selection_llm.complete.return_value = '{"selected": true, "confidence": 0.9}'
...
class TestClaimifyPipelineIntegration(unittest.TestCase):
    ...
    def test_example_from_documentation(self):
        ...
        total_output = sum(len(r.final_claims) + len(r.final_sentences) for r in results)
