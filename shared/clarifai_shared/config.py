...
class ClarifAIConfig:
    ...
    def _apply_host_fallback(host: str) -> str:
        ...
        docker_services = {"postgres", "neo4j", "rabbitmq", "clarifai-core", "vault-watcher", "scheduler"}
        
        # If running in Docker and host is not localhost/127.0.0.1 and not a known service
        if (running_in_docker and 
            host not in docker_services and 
            host not in ("localhost", "127.0.0.1")):
            
            # Check if it's an IP address or external hostname
            if ClarifAIConfig._is_external_host(host):
                logger.info(f"Applying host.docker.internal fallback for external host: {host}")
    ...
    def validate_required_vars(self, required_vars: Optional[List[str]] = None) -> List[str]:
        """
...
def load_config(env_file: Optional[str] = None, 
                validate: bool = True, 
                required_vars: Optional[List[str]] = None) -> ClarifAIConfig:
    """
