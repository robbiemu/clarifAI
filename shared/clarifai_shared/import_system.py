"""
Tier 1 Markdown import system for ClarifAI.
...
def calculate_file_hash(file_path: Path) -> str:
    ...
    try:
        ...
        raise ImportSystemError(f"Cannot calculate hash for {file_path}: {e}")
    
    return sha256_hash.hexdigest()


def is_duplicate_import(file_path: Path, hash_value: str, import_log_dir: Path) -> bool:
    """
...
def record_import(file_path: Path, hash_value: str, import_log_dir: Path, 
...
def write_file_atomically(target_path: Path, content: str) -> None:
    ...
    try:
        ...
        raise ImportSystemError(f"Failed to write file {target_path}: {e}")
...
def convert_file_to_markdowns(input_path: Path, registry: List[Plugin]) -> List[MarkdownOutput]:
    ...
    try:
        ...
        raise ImportSystemError(f"Cannot read file {input_path}: {e}")
...
def format_tier1_markdown(md: MarkdownOutput) -> str:
    ...
    if "<!-- clarifai:title=" in md.markdown_text:
    ...
    lines.extend([
        f"<!-- clarifai:title={md.title} -->",
        f"<!-- clarifai:created_at={metadata.get('created_at', '')} -->",
        f"<!-- clarifai:participants={json.dumps(metadata.get('participants', []))} -->",
        f"<!-- clarifai:message_count={metadata.get('message_count', 0)} -->",
        f"<!-- clarifai:plugin_metadata={json.dumps(metadata.get('plugin_metadata', {}))} -->",
...
def generate_output_filename(md: MarkdownOutput, source_path: Path) -> str:
    ...
    try:
        date_part = datetime.fromisoformat(created_at.replace("Z", "+00:00")).strftime("%Y-%m-%d")
...
class Tier1ImportSystem:
    ...
    def __init__(self, config: Optional[ClarifAIConfig] = None):
        ...
        Args:
            config: ClarifAI configuration (will load from env if not provided)
        """
    ...
    def import_file(self, source_path: Path) -> List[Path]:
        ...
        if is_duplicate_import(source_path, file_hash, self.import_log_dir):
            ...
            raise DuplicateDetectionError(f"File {source_path} is a duplicate (hash: {file_hash})")
        
        # Step 3: Convert using plugins
        try:
            markdown_outputs = convert_file_to_markdowns(source_path, self.plugin_registry)
    ...
    def import_directory(self, source_dir: Path, recursive: bool = True) -> Dict[Path, List[Path]]:
        """
        ...
        logger.info(f"Starting directory import from {source_dir} (recursive={recursive})")
        ...
        logger.info(f"Directory import complete: {len(results)} processed, {duplicates} duplicates, {errors} errors")
        return results