...
class ClaimifyGraphIntegration:
    ...
    def _create_claim_input(self, claim_candidate: ClaimCandidate, source_chunk: SentenceChunk) -> ClaimInput:
        ...
        return ClaimInput(
            ...
            context_complete=claim_candidate.is_atomic,  # Use atomic as proxy for context_complete
...
def create_graph_manager_from_config(config: dict) -> Neo4jGraphManager:
    ...
    """
    # Load ClarifAI config which includes Neo4j settings
    from ..config import load_config
    clarifai_config = load_config(validate=False)
    
    manager = Neo4jGraphManager(config=clarifai_config)
