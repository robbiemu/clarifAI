...
class BaseClaimifyAgent(ABC):
    ...
    def __init__(self, llm: Optional[LLMInterface] = None, config: Optional[ClaimifyConfig] = None):
        self.llm = llm
    ...
    def _log_decision(self, stage: str, decision: str, reasoning: str = "") -> None:
        ...
        if self.config.log_decisions:
            self.logger.info(
                f"Claimify.{stage} decision: {decision}",
                extra={
    ...
    def _log_transformation(self, stage: str, original: str, transformed: str, changes: list) -> None:
        """Log transformations if configured."""
        if self.config.log_transformations:
            self.logger.info(
                f"Claimify.{stage} transformation: {len(changes)} changes made",
                extra={
                    "service": "clarifai-core", 
                    "stage": stage,
    ...
    def _log_timing(self, stage: str, processing_time: float) -> None:
        ...
        if self.config.log_timing:
            self.logger.debug(
                f"Claimify.{stage} processing time: {processing_time:.3f}s",
                extra={
                    "service": "clarifai-core",
                    "stage": stage, 
...
class SelectionAgent(BaseClaimifyAgent):
    ...
    def process(self, context: ClaimifyContext) -> SelectionResult:
        ...
        try:
            ...
            processing_time = time.time() - start_time
            self.logger.error(
                f"Error in selection processing: {e}",
                extra={"service": "clarifai-core", "stage": "selection", "error": str(e)}
            )
    ...
    def _heuristic_selection(self, context: ClaimifyContext) -> SelectionResult:
        ...
        verifiable_indicators = [
            ...
            "error", "exception", "bug", "issue", "problem", "erro",
            # Portuguese error indicators
            "argumento", "tipo", "não pode", "atribuído", "função",
            # Factual statements
            "is", "are", "was", "were", "has", "have", "had",
            # Actions and outcomes
            "caused", "resulted", "occurred", "happened", "found", "reported",
            # Measurements and quantities
            "percent", "%", "number", "count", "amount", "size",
            # Comparisons
            "better", "worse", "more", "less", "faster", "slower",
            # Technical content
            "slice", "none", "int", "__setitem__", "pylance",
        ]
        ...
        indicator_count = sum(1 for indicator in verifiable_indicators if indicator in text_lower)
        
        # Select if we find verifiable content indicators
        is_selected = indicator_count > 0
        confidence = min(0.6 + (indicator_count * 0.1), 0.9)
        
        reasoning = f"Found {indicator_count} verifiable indicators" if is_selected else "No verifiable indicators found"
...
Respond with JSON: {{"selected": true/false, "confidence": 0.0-1.0, "reasoning": "explanation"}}"""
        ...
        try:
            ...
            self.logger.warning(f"LLM selection failed, falling back to heuristics: {e}")
            return self._heuristic_selection(context)
...
class DisambiguationAgent(BaseClaimifyAgent):
    ...
    def process(self, sentence: SentenceChunk, context: ClaimifyContext) -> DisambiguationResult:
        """
        ...
        try:
            ...
            processing_time = time.time() - start_time
            self.logger.error(
                f"Error in disambiguation processing: {e}",
                extra={"service": "clarifai-core", "stage": "disambiguation", "error": str(e)}
            )
    ...
    def _heuristic_disambiguation(self, sentence: SentenceChunk, context: ClaimifyContext) -> DisambiguationResult:
        ...
        for pronoun, replacement in pronouns_to_replace.items():
            if pronoun in disambiguated:
                ...
                changes.append(f"Replaced '{pronoun.strip()}' with '{replacement.strip()}'")
        
        # Add subject inference for sentences starting with verbs
        verb_starters = ["reported", "shows", "indicates", "caused", "resulted", "occurred"]
        ...
        for verb in verb_starters:
            if text_lower.startswith(verb):
                ...
                changes.append(f"Added inferred subject '[The system]' to verb-starting sentence")
                break
    ...
    def _llm_disambiguation(self, sentence: SentenceChunk, context: ClaimifyContext) -> DisambiguationResult:
...
class DecompositionAgent(BaseClaimifyAgent):
    ...
    def process(self, disambiguated_text: str, original_sentence: SentenceChunk) -> DecompositionResult:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Claimify.decomposition completed: {len(valid_claims)} claims, {len(sentence_nodes)} sentences",
                extra={
                    "service": "clarifai-core",
                    "stage": "decomposition", 
            ...
            processing_time = time.time() - start_time
            self.logger.error(
                f"Error in decomposition processing: {e}",
                extra={"service": "clarifai-core", "stage": "decomposition", "error": str(e)}
            )
    ...
    def _heuristic_decomposition(self, text: str) -> DecompositionResult:
        ...
        candidate.reasoning = "Passes all criteria" if candidate.passes_criteria else f"Issues: {', '.join(issues)}"
    ...
    def _check_self_containment(self, text: str) -> bool:
        ...
        ambiguous_refs = [" it ", " this ", " that ", " they ", " these ", " those ", " the error", " the issue"]
        text_lower = text.lower()
        
        # Also check for sentence-starting ambiguous pronouns
        if text_lower.startswith("it ") or text_lower.startswith("this ") or text_lower.startswith("that "):
            return False
    ...
    def _check_verifiability(self, text: str) -> bool:
        ...
        verifiable_patterns = [
            ...
            "is", "are", "was", "were", "has", "have", "had",
            # Events and actions
            "occurred", "happened", "caused", "resulted", "reported",
            # Measurements
            "percent", "%", "number", "size", "count",
        ]
