...
from .agents import SelectionAgent, DisambiguationAgent, DecompositionAgent, LLMInterface
...
class ClaimifyPipeline:
    ...
    def __init__(
        ...
        self.disambiguation_agent = DisambiguationAgent(llm=disambiguation_llm, config=self.config)
        self.decomposition_agent = DecompositionAgent(llm=decomposition_llm, config=self.config)
        
        self.logger.info(
            "Claimify pipeline initialized",
            extra={
                ...
                "config": {
                    ...
                    "disambiguation_model": self.config.get_model_for_stage("disambiguation"),
                    "decomposition_model": self.config.get_model_for_stage("decomposition"),
                }
            }
        )
    ...
    def process_sentences(self, sentences: List[SentenceChunk]) -> List[ClaimifyResult]:
        ...
        """
        if not sentences:
            self.logger.warning("No sentences provided for processing")
            return []
        
        self.logger.info(
            f"Starting Claimify pipeline processing for {len(sentences)} sentences",
            extra={
                "service": "clarifai-core",
                "pipeline": "claimify",
        ...
        for i, sentence in enumerate(sentences):
            try:
                ...
                self.logger.error(
                    f"Error processing sentence {i}: {e}",
                    extra={
                        "service": "clarifai-core",
                        "pipeline": "claimify", 
                        "sentence_index": i,
        ...
        self.logger.info(
            f"Claimify pipeline completed: {processed_count}/{len(sentences)} processed, "
    ...
    def process_sentence(self, context: ClaimifyContext) -> ClaimifyResult:
        ...
        try:
            # Stage 1: Selection
            self.logger.debug(
                f"Processing sentence through Selection: {sentence.text[:50]}...",
                extra={
                    "service": "clarifai-core",
                    "pipeline": "claimify",
                    "stage": "selection",
            ...
            self.logger.debug(
                f"Processing sentence through Disambiguation: {sentence.text[:50]}...",
                extra={
                    "service": "clarifai-core", 
                    "pipeline": "claimify",
            ...
            self.logger.debug(
                f"Processing sentence through Decomposition: {disambiguation_result.disambiguated_text[:50]}...",
                extra={
                    "service": "clarifai-core",
                    "pipeline": "claimify", 
                    "stage": "decomposition",
            ...
            decomposition_result = self.decomposition_agent.process(
                disambiguation_result.disambiguated_text, 
                sentence
            )
            ...
            self.logger.error(
                f"Error in pipeline processing: {e}",
                extra={
                    "service": "clarifai-core",
                    "pipeline": "claimify",
                    "sentence_id": sentence.chunk_id,
    ...
    def _build_context_window(
        ...
        end_f = min(len(all_sentences), current_index + 1 + self.config.context_window_f)
        ...
        self.logger.debug(
            f"Built context window: p={len(preceding)}, f={len(following)}",
            extra={
                "service": "clarifai-core",
                "pipeline": "claimify", 
    ...
    def get_pipeline_stats(self, results: List[ClaimifyResult]) -> Dict[str, Any]:
        ...
        selection_times = [r.selection_result.processing_time for r in results 
                          if r.selection_result and r.selection_result.processing_time]
        disambiguation_times = [r.disambiguation_result.processing_time for r in results 
                               if r.disambiguation_result and r.disambiguation_result.processing_time]
        decomposition_times = [r.decomposition_result.processing_time for r in results 
                              if r.decomposition_result and r.decomposition_result.processing_time]
        total_times = [r.total_processing_time for r in results if r.total_processing_time]
        ...
        stats = {
            ...
            "timing": {
                "selection_avg": sum(selection_times) / len(selection_times) if selection_times else 0,
                "disambiguation_avg": sum(disambiguation_times) / len(disambiguation_times) if disambiguation_times else 0,
                "decomposition_avg": sum(decomposition_times) / len(decomposition_times) if decomposition_times else 0,
