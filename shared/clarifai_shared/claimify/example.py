...
def main():
    ...
    print(
        f"Processing {len(example_sentences)} sentences through Claimify pipeline...\n"
    )
    ...
    for i, result in enumerate(results):
        ...
        if result.decomposition_result:
            ...
            for j, claim in enumerate(decomp.valid_claims):
                ...
                print(
                    f"      Atomic: {claim.is_atomic}, Self-contained: {claim.is_self_contained}, Verifiable: {claim.is_verifiable}"
                )

            # Show sentence nodes
            for j, sentence_node in enumerate(decomp.sentence_nodes):
                print(
                    f'    Sentence {j + 1}: "{sentence_node.text}" (→ :Sentence node)'
                )
    ...
    print(
        "4. ✅ Distinguished between valid claims (→ :Claim nodes) and incomplete statements (→ :Sentence nodes)"
    )
