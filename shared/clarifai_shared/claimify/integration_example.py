...
def load_config_example():
    ...
    print(f"  Logging: decisions={config.log_decisions}, transformations={config.log_transformations}")
...
def demonstrate_pipeline_with_config():
    ...
    print(f"Processing {len(sentences)} sentences with context window p={config.context_window_p}, f={config.context_window_f}...")
    ...
    for i, result in enumerate(results):
        ...
        if result.selection_result:
            status = "✅ SELECTED" if result.selection_result.is_selected else "❌ REJECTED"
            print(f"  Selection: {status}")
            
        if result.disambiguation_result:
            print(f"  Disambiguated: {result.disambiguation_result.disambiguated_text[:60]}...")
...
def demonstrate_configuration_flexibility():
    ...
    print(f"  Context: p={large_context_config.context_window_p}, f={large_context_config.context_window_f}")
    print(f"  Models: {large_context_config.get_model_for_stage('selection')}, "
    ...
    print("\nExample 2: Production configuration")
    print(f"  Optimized for: cost-effectiveness and reliability")
    ...
    print(f"  Logging: minimal but includes timing")
