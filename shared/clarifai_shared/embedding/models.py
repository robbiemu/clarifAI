...
class EmbeddingGenerator:
    ...
    def __init__(self, config: Optional[ClarifAIConfig] = None, model_name: Optional[str] = None):
        """
    ...
    def _initialize_embedding_model(self) -> BaseEmbedding:
        ...
        try:
            ...
            logger.info(f"Successfully initialized embedding model: {self.model_name} on {device}")
    ...
    def _embed_texts_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            logger.debug(f"Processing embedding batch {i//batch_size + 1}: {len(batch_texts)} texts")
    ...
    def validate_embeddings(self, embedded_chunks: List[EmbeddedChunk]) -> Dict[str, Any]:
        ...
        for chunk in embedded_chunks:
            if any(not (isinstance(x, (int, float)) and x != float('inf') and x != float('-inf') and x == x) 
        ...
        logger.info(f"Embedding validation: {validation_report['status']} - "
