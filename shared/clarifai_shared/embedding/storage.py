...
from sqlalchemy import create_engine, text, MetaData, Table, Column, String, Integer, JSON
...
class ClarifAIVectorStore:
    ...
    def __init__(self, config: Optional[ClarifAIConfig] = None):
        ...
        self.connection_string = config.postgres.get_connection_url("postgresql+psycopg2")
    ...
    def store_embeddings(self, embedded_chunks: List[EmbeddedChunk]) -> VectorStoreMetrics:
        """
        ...
        try:
            ...
            logger.info(f"Storage complete: {successful_inserts} successful, {failed_inserts} failed")
    ...
    def similarity_search(
        ...
        """
        logger.debug(f"Performing similarity search: query='{query_text[:50]}...', top_k={top_k}")
    ...
    def get_chunk_by_id(self, clarifai_block_id: str, chunk_index: int) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to retrieve chunk {clarifai_block_id}[{chunk_index}]: {e}")
            return None
    ...
    def get_chunks_by_block_id(self, clarifai_block_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Retrieved {len(chunks)} chunks for block {clarifai_block_id}")
            return chunks
            
        except Exception as e:
            logger.error(f"Failed to retrieve chunks for block {clarifai_block_id}: {e}")
            return []
        ...
        try:
            with self.engine.connect() as conn:
                ...
                count_query = text(f"""
                    ...
                    FROM {self.config.embedding.collection_name}
                """)
                ...
                # Get table size
                size_query = text(f"""
                    SELECT pg_size_pretty(pg_total_relation_size('{self.config.embedding.collection_name}'))
                """)
    ...
    def _initialize_pgvector_store(self) -> PGVectorStore:
        ...
        try:
            ...
            logger.info(f"Initialized PGVectorStore with table: {self.config.embedding.collection_name}")
            return vector_store
    ...
    def _convert_to_documents(self, embedded_chunks: List[EmbeddedChunk]) -> List[Document]:
        """
