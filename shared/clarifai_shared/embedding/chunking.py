...
class UtteranceChunker:
    ...
    def chunk_tier1_blocks(self, tier1_content: str) -> List[ChunkMetadata]:
        ...
        for block in utterance_blocks:
            chunks = self.chunk_utterance_block(block["text"], block["clarifai_id"])
    ...
    def chunk_utterance_block(
        self, text: str, clarifai_block_id: str
    ) -> List[ChunkMetadata]:
        """
        ...
        logger.debug(
            f"Generated {len(chunk_metadata)} final chunks for block {clarifai_block_id}"
        )
    ...
    def _parse_tier1_blocks(self, tier1_content: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if (
                not line
                or line.startswith("<!-- clarifai:title=")
                or line.startswith("<!-- clarifai:created_at=")
            ):
                continue
