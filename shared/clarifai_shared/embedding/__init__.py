...
class EmbeddingPipeline:
    ...
    def process_single_block(
        self, text: str, clarifai_block_id: str, replace_existing: bool = True
    ) -> EmbeddingResult:
        """
        ...
        try:
            ...
            if replace_existing:
                ...
                if deleted_count > 0:
                    logger.debug(
                        f"Removed {deleted_count} existing chunks for block {clarifai_block_id}"
                    )
            ...
            logger.info(
                f"Single block processing completed: {clarifai_block_id}, success={success}"
            )
            return result
