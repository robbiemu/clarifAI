...
- ClarifAIVectorStore: Stores vectors in PostgreSQL with pgvector
...
class EmbeddingPipeline:
    ...
    def __init__(self, config: Optional[ClarifAIConfig] = None):
        ...
        self.vector_store = ClarifAIVectorStore(config)
    ...
    def process_tier1_content(self, tier1_content: str) -> EmbeddingResult:
        ...
        try:
            ...
            validation_report = self.embedding_generator.validate_embeddings(embedded_chunks)
            if validation_report["status"] == "warning":
                errors.append(f"Embedding validation warnings: {validation_report}")
            
            # Step 4: Store in vector database
            logger.debug(f"Step 4: Storing {len(embedded_chunks)} embeddings in vector store")
            storage_metrics = self.vector_store.store_embeddings(embedded_chunks)
            
            # Calculate final results
            success = (storage_metrics.successful_inserts > 0 and 
                      storage_metrics.failed_inserts == 0)
    ...
    def process_single_block(
        ...
        Returns:
            EmbeddingResult with processing metrics and status
        """
        logger.info(f"Processing single block: {clarifai_block_id}")
        errors = []
        
        try:
            # Remove existing chunks if requested
            if replace_existing:
                deleted_count = self.vector_store.delete_chunks_by_block_id(clarifai_block_id)
                if deleted_count > 0:
                    logger.debug(f"Removed {deleted_count} existing chunks for block {clarifai_block_id}")
            
            # Step 1: Chunk the block
            chunks = self.chunker.chunk_utterance_block(text, clarifai_block_id)
            
            if not chunks:
                logger.warning(f"No chunks generated for block {clarifai_block_id}")
                return EmbeddingResult(
                    success=False,
                    total_chunks=0,
                    embedded_chunks=0,
            ...
            logger.info(f"Single block processing completed: {clarifai_block_id}, success={success}")
            return result
            
        except Exception as e:
            error_msg = f"Single block processing failed for {clarifai_block_id}: {e}"
            logger.error(error_msg)
            errors.append(error_msg)
