...
try:
    from clarifai_shared.embedding import EmbeddingPipeline, EmbeddingResult
...
def mock_database_components():
    ...
    mock_store.similarity_search.return_value = [
        ({"clarifai_block_id": "blk_demo01", "chunk_index": 0, "text": "Sample text 1"}, 0.95),
        ({"clarifai_block_id": "blk_demo02", "chunk_index": 0, "text": "Sample text 2"}, 0.87),
    ]
    
    return mock_store


def run_chunking_demo(pipeline: EmbeddingPipeline, tier1_content: str) -> Dict[str, Any]:
...
def run_embedding_demo(pipeline: EmbeddingPipeline, chunks: list, mock_db: bool = False) -> Dict[str, Any]:
    ...
    print(f"Embedding {len(chunks)} chunks using model: {pipeline.embedding_generator.model_name}")
    ...
    try:
        ...
        if embedded_chunks:
            ...
            # Validate embeddings
            validation_report = pipeline.embedding_generator.validate_embeddings(embedded_chunks)
        ...
        if not mock_db:
            ...
            print("Try running with --mock-db for testing without actual embedding models.")
        return {"embedded_chunks": 0, "error": str(e)}


def run_storage_demo(pipeline: EmbeddingPipeline, embedded_chunks: list, mock_db: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        print("Check your PostgreSQL configuration and ensure pgvector extension is installed.")
        return {"stored_chunks": 0, "failed_chunks": len(embedded_chunks), "error": str(e)}


def run_similarity_search_demo(pipeline: EmbeddingPipeline, mock_db: bool = False):
...
def run_pipeline_demo(pipeline: EmbeddingPipeline, tier1_content: str, mock_db: bool = False) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument("--mock-db", action="store_true", help="Use mock database for testing")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--test-similarity", action="store_true", help="Run similarity search tests")
    ...
    print(f"Chunking: {demo_results.get('chunking', {}).get('total_chunks', 0)} chunks generated")
    print(f"Embedding: {demo_results.get('embedding', {}).get('embedded_chunks', 0)} chunks embedded")
    print(f"Pipeline: {'Success' if demo_results.get('pipeline', {}).get('success') else 'Failed'}")
