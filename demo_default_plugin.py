...
DURATION: 5m0s
    ...
    try:
        ...
        if outputs:
            ...
            print(
                f"   Plugin Used: {output.metadata['plugin_metadata']['source_format']}"
            )
...
jane: Just waiting on the design review.
    ...
    try:
        ...
        if outputs:
            print("✅ DefaultPlugin successfully processed the file")
            print(
                f"📊 Extracted conversation with {len(outputs[0].metadata['participants'])} participants"
            )
        else:
...
def demo_plugin_interface():
    ...
    for i, test_input in enumerate(test_inputs, 1):
        ...
        print(
            f"  {i}. '{test_input[:30]}{'...' if len(test_input) > 30 else ''}' -> {accepts}"
        )
