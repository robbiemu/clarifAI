...
def test_vault_sync_with_fixtures():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(tier3_blocks) == 1, f"Expected 1 block in tier3, got {len(tier3_blocks)}"
        ...
        assert len(mixed_blocks) == 4, f"Expected 4 blocks in mixed content, got {len(mixed_blocks)}"
        ...
        assert len(no_id_blocks) == 0, f"Expected 0 blocks in no-ID file, got {len(no_id_blocks)}"
...
def test_file_structure_discovery():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (vault_path / "conversations" / "meeting_001.md").write_text("# Meeting\nContent <!-- clarifai:id=blk_001 -->")
        (vault_path / "conversations" / "brainstorm.md").write_text("# Brainstorm\nIdeas <!-- clarifai:id=blk_002 -->")
        (vault_path / "concepts" / "important_concept.md").write_text("# Concept\nDescription <!-- clarifai:id=concept_001 -->")
        (vault_path / "summaries" / "weekly_summary.md").write_text("# Summary\nOverview")  # No clarifai:id
        ...
        assert len(tier1_files) == 2, f"Expected 2 tier1 files, found {len(tier1_files)}"
