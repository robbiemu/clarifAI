...
def test_service_modules_exist():
    ...
    service_modules = {
        ...
        "vault-watcher": "vault_watcher",
        "scheduler": "scheduler",
        "clarifai-ui": "clarifai_ui"
    }

    for service_name, module_name in service_modules.items():
        service_dir = services_dir / service_name / module_name
        assert service_dir.exists(), f"Module directory {module_name} not found in {service_name}"

        # Check for __init__.py
        init_file = service_dir / "__init__.py"
        assert init_file.exists(), f"__init__.py not found in {service_name}/{module_name}"
...
def test_shared_package():
    ...
    try:
        ...
        pytest.fail(f"Failed to import shared package: {e}")
