...
class ConfigurationManager:
    ...
    def __init__(self, config_path: str = "/home/runner/work/aclarai/aclarai/settings/aclarai.config.yaml"):
        self.config_path = Path(config_path)
        self.default_config_path = Path("/home/runner/work/aclarai/aclarai/shared/aclarai_shared/aclarai.config.default.yaml")
    ...
    @staticmethod
    def _deep_merge_configs(default: Dict[str, Any], user: Dict[str, Any]) -> Dict[str, Any]:
        """Deep merge user configuration over default configuration."""
        result = copy.deepcopy(default)
        
        def _merge_recursive(base_dict: Dict[str, Any], override_dict: Dict[str, Any]) -> None:
...
def validate_model_name(model_name: str) -> Tuple[bool, str]:
    ...
    valid_patterns = [
        ...
        lambda x: x.startswith("sentence-transformers/"),  # HuggingFace sentence transformers
...
def validate_window_param(value: int, min_val: int = 0, max_val: int = 10) -> Tuple[bool, str]:
...
def create_configuration_panel() -> gr.Blocks:
    ...
    def load_current_config() -> Tuple[str, str, str, str, str, str, str, str, str, str, str, float, float, int, int]:
        ...
        try:
            ...
            claimify_disambiguation = claimify_config.get("disambiguation") or claimify_default
            claimify_decomposition = claimify_config.get("decomposition") or claimify_default
            ...
            utterance_embedding = embedding_config.get("utterance", "sentence-transformers/all-MiniLM-L6-v2")
            ...
            return (
                claimify_default, claimify_selection, claimify_disambiguation, claimify_decomposition,
                concept_linker, concept_summary, subject_summary, trending_concepts_agent, fallback_plugin,
                utterance_embedding, concept_embedding,
                concept_merge, claim_link_strength,
                window_p, window_f
            )
            ...
            return (
                ...
                "gpt-3.5-turbo", "gpt-4", "gpt-3.5-turbo", "gpt-4", "gpt-3.5-turbo",
    ...
    def save_configuration(
        claimify_default: str, claimify_selection: str, claimify_disambiguation: str, claimify_decomposition: str,
        concept_linker: str, concept_summary: str, subject_summary: str, trending_concepts_agent: str, fallback_plugin: str,
        utterance_embedding: str, concept_embedding: str,
        concept_merge: float, claim_link_strength: float,
        window_p: int, window_f: int
    ) -> str:
        ...
        try:
            ...
            current_config["model"]["claimify"] = {
                ...
                "selection": claimify_selection.strip() if claimify_selection.strip() != claimify_default.strip() else None,
                "disambiguation": claimify_disambiguation.strip() if claimify_disambiguation.strip() != claimify_default.strip() else None,
                "decomposition": claimify_decomposition.strip() if claimify_decomposition.strip() != claimify_default.strip() else None,
            }
            ...
            current_config["model"]["trending_concepts_agent"] = trending_concepts_agent.strip()
    ...
    with gr.Blocks(title="aclarai - Configuration Panel", theme=gr.themes.Soft()) as interface:
        ...
        def reload_configuration():
            ...
            try:
                ...
                return initial_values + (f"❌ **Error reloading configuration:** {str(e)}",)
        
        # Save button click handler
        save_btn.click(
            fn=save_configuration,
            inputs=[
                claimify_default_input, claimify_selection_input, claimify_disambiguation_input, claimify_decomposition_input,
                concept_linker_input, concept_summary_input, subject_summary_input, trending_concepts_agent_input, fallback_plugin_input,
                utterance_embedding_input, concept_embedding_input,
                concept_merge_input, claim_link_strength_input,
                window_p_input, window_f_input
            ],
            outputs=[save_status]
        )
