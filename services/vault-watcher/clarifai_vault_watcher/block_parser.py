...
class BlockParser:
    ...
    def _extract_inline_blocks(self, content: str) -> List[ClarifAIBlock]:
        ...
        for match in self.INLINE_BLOCK_PATTERN.finditer(content):
            ...
            if not after_comment:
                ...
                content_before_comment = comment_line[:comment_line.find('<!--')].strip()
    ...
    def _extract_block_content(self, content: str, comment_match: re.Match) -> Optional[Dict]:
        """
    ...
    def compare_blocks(
        self, 
        old_blocks: List[ClarifAIBlock], 
        new_blocks: List[ClarifAIBlock]
    ) -> Dict[str, Dict]:
        """
        ...
        for block_id, new_block in new_block_map.items():
            if block_id not in old_block_map:
                ...
                if (old_block.content_hash != new_block.content_hash or 
