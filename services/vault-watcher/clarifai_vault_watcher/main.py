...
class VaultWatcherService:
    ...
    def __init__(self, config):
        ...
        self.file_watcher = BatchedFileWatcher(
            ...
            batch_interval=2.0,  # 2 second batching
            max_batch_size=50,   # Process after 50 events
        ...
        self.publisher = DirtyBlockPublisher(
            ...
            rabbitmq_user=getattr(config, 'rabbitmq_user', None),
            rabbitmq_password=getattr(config, 'rabbitmq_password', None),
    ...
    def _handle_file_modified(self, file_path: Path) -> None:
        ...
        try:
            ...
            if any(dirty_blocks[key] for key in ['added', 'modified', 'deleted']):
                ...
                total_changes = sum(len(dirty_blocks[key]) for key in ['added', 'modified', 'deleted'])
...
def main():
    ...
    try:
        ...
        if not os.path.exists(config.vault_path):
            logger.warning(
                f"vault_watcher.main: Vault path does not exist, creating: {config.vault_path}",
                extra={
                    "service": "vault-watcher",
                    "filename.function_name": "main.main",
                    "vault_path": config.vault_path
